...
int main(int argc, char *argv[])
{
	...
	for(;;) {
		...
		if(command_status == 2) {
			...
			delete_binary(a_head);			//構造体二分木容量開放
			printf("done.\n");
			exit(EXIT_SUCCESS);
		  ...
		  char *args[])			/* 引数へのポインタ配列 */
{
	...
	char *args_temp[COMMANDLEN];
	char wild_card[WILDCARDNUM][DIRLEN];
	...
	if(arg_index > 0){				//ワイルドカード機能
		...
		if(args[i] != NULL){
			...
			while ((dp = readdir(dir)) != NULL) {
				strcpy(&wild_card[j][0], dp->d_name);
				args[arg_index] = &wild_card[j][0];
				arg_index++;
				j++;
			}
			j = 0;
			while(args_temp[j] != NULL){
				args[arg_index] = args_temp[j];
				arg_index++;
				j++;
			}
			args[arg_index] = NULL;
					 ...
					 int command_status)	 /* コマンドの状態 */
{
	...
	/******** Your Program ********/
		int wait = waitpid(pid, &status, 0); //子プロセスのプロセスIDを指定して、終了を待つ
		if(wait < 0){
			printf("wait failure\n");
			exit(EXIT_FAILURE);
		}
